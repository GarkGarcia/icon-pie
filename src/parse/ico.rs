...
pub fn parse(it: &mut TokenStream, n_entries: usize) -> Result<Command, Error> {
    ...
    while let Some(&(c, token)) = it.peek() {
        match token {
            Token::Flag(Flag::Entry) => add_entry::<Ico, _>(it, &mut entries, entry_adder)?,
            Token::Flag(Flag::Output) => return command(it, entries),
            _ => return syntax!(SyntaxError::UnexpectedToken(c))
        }
    }

    Ok(Command::Ico(entries, Output::Stdout))
}

fn command(it: &mut TokenStream, entries: Entries<Ico>) -> Result<Command, Error> {
    it.next();
    match it.peek() {
        Some(&(_, Token::Path(path))) => expect_end(it, Command::Ico(entries, Output::Path(path.clone()))),
        Some(&(c, _)) => syntax!(SyntaxError::UnexpectedToken(c)),
        None => syntax!(SyntaxError::UnexpectedEnd)
    }
}

fn entry_adder(it: &mut TokenStream, entries: &mut Entries<Ico>, path: &PathBuf) -> Result<(), Error> {
    ...
    for size in sizes {
        if size < 256 {
            if let Some(key) = <Ico as Icon>::Key::new(size as u8) {
